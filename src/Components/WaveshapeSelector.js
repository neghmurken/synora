import React, { useContext } from 'react'
import styled from 'styled-components'
import { map } from 'ramda'
import { getParam, setParam } from '../state'
import { SynthInstrumentContext } from './Engine'

export const WaveshapeSelector = ({ oscillatorNumber }) => {
  const [state, dispatch] = useContext(SynthInstrumentContext)
  const waveshapes = ['sine', 'square', 'triangle', 'sawtooth']

  return (
    <div>
      <p>
        Type&nbsp;
        <Select defaultValue={getParam(state, `osc${oscillatorNumber}_type`)} onChange={event => {
          setParam(dispatch, `osc${oscillatorNumber}_type`, event.target.value)
        }}>
          {map(shape => <option key={shape} value={shape}>{shape}</option>)(waveshapes)}
        </Select>
      </p>
    </div>
  )
}

const Select = styled.select`
  background: black;
  color: white;
  border: 1px solid gray;
  outline: none;

  option {
    &[value="sine"] {
      background-image: url("data:image/svg+xml;base64,");
    }
  }
`

// tri: background-image: url("data:image/svg+xml;base64,");
// sin:
// sqr: background-image: url("data:image/svg+xml;base64,");
// saw: background-image: url("data:image/svg+xml;base64,");
